//flags to enable generate a logs if some uncaught exception be trigger, on some error and save all inside of /reports directory
...
function start(db) {
  ...
  Http.Server.prototype.emit = function (type, req, res) {
    ...
    logRequests('start')
    res.on('finish', () => logRequests('finish'))
